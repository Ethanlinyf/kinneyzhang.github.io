...
我的博客为亮色主题，晚间浏览时光线太强，于是想实现在亮色与暗色主题间的自由切换。实现原理参考了 https://gongzhitaao.org/orgcss/ ，即通过改变引入的css文件链接来实现主题切换。话不多说，先来看看效果：

https://s23.aconvert.com/convert/p3r68-cdx67/hshkd-z0kzy.gif
...
下面是具体的步骤：
 ...
 3. 在 =</body>= 标签之前引入 =switchTheme= 函数，通过改变css文件切换主题。实现单个按钮循环切换主题，设置 =flag= 改变状态，状态存储在sessionStorage中。
 ...
 4. 这样就可以实现亮/暗两个主题的切换了。但是有个问题，主题的状态不能保持，刷新页面会恢复到默认的主题状态。我通过sessionStorage来存储主题状态，改写后完整的js如下：
